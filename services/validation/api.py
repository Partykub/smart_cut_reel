"""FastAPI adapter for the Phase 1 validation service."""
...
def create_app(service: ValidationService | None = None) -> FastAPI:
    app = FastAPI(title="Smart Cut Reel Validation Service", version="0.1.0")
    validation_service = service or ValidationService()

    @app.post("/run")
    async def run(request: ApiRunRequest) -> ApiRunResponse:
        try:
            response = validation_service.run(build_context(request.to_runtime()))
